...
#### 游戏逻辑：
- `bevy_animation` → `crates/bevy_animation/` - 动画系统
- `bevy_transform` → `crates/bevy_transform/` - 变换系统
- `bevy_input` → `crates/bevy_input/` - 输入处理
- `bevy_picking` → `crates/bevy_picking/` - 交互拾取

#### 调试和诊断：
- `bevy_diagnostic` → `crates/bevy_diagnostic/` - 诊断系统
- `bevy_gizmos` → `crates/bevy_gizmos/` - 调试绘制
...
#### 预期产出：
- `crates/bevy_transform/` - 变换系统
- `crates/bevy_input/` - 输入处理
- `crates/bevy_animation/` - 动画控制
- `crates/bevy_picking/` - 交互拾取
