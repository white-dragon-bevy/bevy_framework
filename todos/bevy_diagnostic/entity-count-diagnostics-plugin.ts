...
export class EntityCountDiagnosticsPlugin implements Plugin {
	 ...
	 * @returns 插件实例
	 */
	static new(maxHistoryLength: number): EntityCountDiagnosticsPlugin {
	...
	build(app: App): void {
		app.registerDiagnostic(
			Diagnostic.new(EntityCountDiagnosticsPlugin.ENTITY_COUNT).withMaxHistoryLength(
				this.maxHistoryLength,
			),
		).addSystems(Update, EntityCountDiagnosticsPlugin.diagnosticSystem);
	}
	 ...
	 * @param world - ECS世界
	 */
	static diagnosticSystem(world: World): void {
