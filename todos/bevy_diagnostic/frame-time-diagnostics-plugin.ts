...
export class FrameTimeDiagnosticsPlugin implements Plugin {
	 ...
	 * @returns 插件实例
	 */
	static new(maxHistoryLength: number): FrameTimeDiagnosticsPlugin {
	...
	build(app: App): void {
		app.registerDiagnostic(
			...
			.registerDiagnostic(
				Diagnostic.new(FrameTimeDiagnosticsPlugin.FPS)
					.withMaxHistoryLength(this.maxHistoryLength)
					.withSmoothingFactor(this.smoothingFactor),
			)
			.registerDiagnostic(
				Diagnostic.new(FrameTimeDiagnosticsPlugin.FRAME_COUNT)
					.withSmoothingFactor(0.0)
					.withMaxHistoryLength(0),
			)
			.addSystems(Update, FrameTimeDiagnosticsPlugin.diagnosticSystem);
	}
	 ...
	 */
	static diagnosticSystem(world: World, deltaTime: number): void {
