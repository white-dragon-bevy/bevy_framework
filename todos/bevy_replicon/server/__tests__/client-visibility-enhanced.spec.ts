...
export = () => {
	describe("ClientVisibility Enhanced Features", () => {
		...
		describe("Change Tracking", () => {
			...
			// TODO: Fix change tracking tests - currently failing due to cache interaction
		...
		describe("Automatic Cleanup", () => {
			...
			it("should cleanup removed entities", () => {
				...
				// 验证被移除的实体的可见客户端列表为空（清理后不应再有映射）
				const visibleClients2 = visibility.getVisibleClients(testEntities[2]);
				const visibleClients3 = visibility.getVisibleClients(testEntities[3]);
				expect(visibleClients2.size()).to.equal(0);
				expect(visibleClients3.size()).to.equal(0);
			});
		...
		describe("Debug and Monitoring", () => {
			...
			it("should track query and update performance", () => {
				...
				const perfStats = visibility.getPerformanceStats();
				// computeChanges 会对每个活跃客户端调用 isEntityVisibleToClient
				// testClients 有 3 个客户端，所以总查询 = 5 (手动) + 3 (computeChanges) = 8
				expect(perfStats.totalQueries).to.equal(8);
