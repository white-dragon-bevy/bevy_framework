...
export = () => {
	...
	describe("GlobalTransform Component", () => {
		...
		it("should transform point from local to world space", () => {
			const transformModule = require(script.Parent!.Parent!.WaitForChild("src").WaitForChild("components").WaitForChild("global-transform") as ModuleScript) as any;
			const transformPoint = transformModule.transformPoint;
		...
		it("should get forward, right, and up vectors", () => {
			const transformModule = require(script.Parent!.Parent!.WaitForChild("src").WaitForChild("components").WaitForChild("global-transform") as ModuleScript) as any;
			const getForward = transformModule.getForward;
			const getRight = transformModule.getRight;
			const getUp = transformModule.getUp;
			const globalTransform = createGlobalTransform();
