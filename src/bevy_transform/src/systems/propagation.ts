	...
	for (const [entity, transform] of world.query(Transform)) {
		...
		if (needsUpdate) {
			...
			while (current !== undefined) {
				...
				// 获取父级
				const parentData: { entity: number } | undefined = world.get(current as AnyEntity, Parent);
...
function propagateRecursive(
	...
	// 检查是否需要更新
	const treeChanged = world.get(entity as AnyEntity, TransformTreeChanged);
	...
	if (shouldUpdate && transform) {
		...
		// 传播到子级
		const childrenData = world.get(entity as AnyEntity, Children);
		if (childrenData) {
		...
		const globalTransform = world.get(entity as AnyEntity, GlobalTransform);
		if (globalTransform) {
			const childrenData = world.get(entity as AnyEntity, Children);
			if (childrenData) {
				for (const childEntity of childrenData.entities) {
...
export function propagateParentTransforms(world: World): void {
	...
	for (const rootEntity of rootEntities) {
		const transform = world.get(rootEntity as AnyEntity, Transform);
		const globalTransform = world.get(rootEntity as AnyEntity, GlobalTransform);

		if (transform && globalTransform) {
			const treeChanged = world.get(rootEntity as AnyEntity, TransformTreeChanged);
			const hasChanged = treeChanged !== undefined;
			...
			if (hasChanged) {
				...
				// 传播到子级
				const childrenData = world.get(rootEntity as AnyEntity, Children);
				if (childrenData) {
					for (const childEntity of childrenData.entities) {
						propagateRecursive(world, childEntity, newGlobalTransform, true);
					}
				...
				const childrenData = world.get(rootEntity as AnyEntity, Children);
				if (childrenData) {
					const currentGlobalTransform = world.get(rootEntity as AnyEntity, GlobalTransform);
