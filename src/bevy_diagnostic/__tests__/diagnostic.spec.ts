...
import { isNaNPolyfill } from "../../utils/string-polyfills";
...
export = () => {
	...
	describe("Diagnostic", () => {
		...
		it("should respect max history length", () => {
			const path = DiagnosticPath.constNew("test");
			const diagnostic = Diagnostic.create(path).withMaxHistoryLength(3);
		...
		it("should handle NaN values", () => {
			...
			expect(isNaNPolyfill(values[1])).to.equal(true);
		});
	...
	describe("DiagnosticsStore", () => {
		...
		it("should iterate over diagnostics", () => {
			...
			store.add(Diagnostic.create(path1));
			store.add(Diagnostic.create(path2));
