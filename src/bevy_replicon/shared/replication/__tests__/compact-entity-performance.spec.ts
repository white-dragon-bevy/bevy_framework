...
export = () => {
	describe("CompactEntity Performance Tests", () => {
		...
		it("should compare favorably against JSON - 10 mappings", () => {
			...
			// 验证优化效果
			expect(result.compressionRatio < 0.1).to.equal(true); // 至少10倍压缩
			expect(result.bytesSaved > 0).to.equal(true);
			expect(result.performanceRatio > 0.5).to.equal(true); // 性能不应显著下降
		});
		...
		it("should compare favorably against JSON - 1000 mappings", () => {
			...
			// 验证优化效果
			expect(result.compressionRatio < 0.04).to.equal(true); // 至少25倍压缩
			expect(result.bytesSaved > 40000).to.equal(true); // 大幅节省
			expect(result.performanceRatio > 0.2).to.equal(true); // 性能可接受

			// 大规模测试的额外验证
			expect(result.compactSerializeTime < 100).to.equal(true); // 序列化时间 < 100ms
			expect(result.compactDeserializeTime < 100).to.equal(true); // 反序列化时间 < 100ms
		});
